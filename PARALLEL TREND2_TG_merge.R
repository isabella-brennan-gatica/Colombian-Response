...
df_in_log <- read_dta ("INFORMAL_LOG_WAGE_TG_QUART_regression.dta")
df1 <- df_in_log %>%
  filter(term %in% params) %>%   
  select(-estimate,-statistic,-Estimate,-StdERROR,-tVALUE,-conflow,-confhigh)%>%
  rename("Informal_Log_Wage"= "pvalue")


df_in_wag <- read_dta ("INFORMAL_THO_MONTHLY_WAGE_TG_QUART_regression.dta")
df2 <- df_in_wag %>%
  filter(term %in% params) %>%   
  select(-estimate,-statistic,-Estimate,-StdERROR,-tVALUE,-conflow,-confhigh)%>%
  rename("Informal_Monthly_Wage"= "pvalue")

df_in_hrs <- read_dta ("INFORMAL_HRS_WRKD_TG_QUART_regression.dta")
df3 <- df_in_hrs %>%
  filter(term %in% params) %>%   
  select(-estimate,-statistic,-Estimate,-StdERROR,-tVALUE,-conflow,-confhigh)%>%
...
df_fo_log <- read_dta ("FORMAL_LOG_WAGE_TG_QUART_regression.dta")
df4 <- df_fo_log %>%
  filter(term %in% params) %>%   
  select(-estimate,-statistic,-Estimate,-StdERROR,-tVALUE,-conflow,-confhigh)%>%
...
df_fo_wag <- read_dta ("FORMAL_THO_MONTHLY_WAGE_TG_QUART_regression.dta")
df5 <- df_fo_wag %>%
  filter(term %in% params) %>%   
  select(-estimate,-statistic,-Estimate,-StdERROR,-tVALUE,-conflow,-confhigh)%>%
  rename("Formal_monthly_wage"= "pvalue")

df_fo_hrs <-read_dta("FORMAL_HRS_WRKD_TG_QUART_regression.dta")
df6 <- df_fo_hrs %>%
  filter(term %in% params) %>%   
  select(-estimate,-statistic,-Estimate,-StdERROR,-tVALUE,-conflow,-confhigh)%>%
  rename("Formal_hrs_week"= "pvalue")

df_full_log <- read_dta ("FULL_LOG_WAGE_TG_QUART_regression.dta")
df7 <- df_fo_log %>%
  filter(term %in% params) %>%   
  select(-estimate,-statistic,-Estimate,-StdERROR,-tVALUE,-conflow,-confhigh)%>%
...
df_full_wag <- read_dta ("FULL_THO_MONTHLY_WAGE_TG_QUART_regression.dta")
df8 <- df_fo_wag %>%
  filter(term %in% params) %>%   
  select(-estimate,-statistic,-Estimate,-StdERROR,-tVALUE,-conflow,-confhigh)%>%
  rename("Full_monthly_wage"= "pvalue")

df_full_hrs <-read_dta("FULL_HRS_WRKD_TG_QUART_regression.dta")
df9 <- df_fo_hrs %>%
  filter(term %in% params) %>%   
  select(-estimate,-statistic,-Estimate,-StdERROR,-tVALUE,-conflow,-confhigh)%>%
...
setwd("C:/Users/.../Colombia/GEIH DATA/PARALLEL TREND ASSUMPTION/TG/")
write_dta(merged_df, "parallel_Results_TG_Merged.dta") 
